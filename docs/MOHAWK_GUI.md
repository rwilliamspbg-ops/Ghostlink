...
The diagnostics JSON includes machine-readable interpreter and environment fields such as
`python`, `python_source`, `has_display`, `xvfb_available`, `missing_python_modules`,
`python_module_probe_error`, `linux_libgl_present`, and `linux_libxkbcommon_present`.

Validate that JSON structurally:

```bash
python3 scripts/validate_gui_diagnostics.py --report ./tmp/gui-diagnostics.json --allow-headless --require-python-source
```

Render a compact markdown summary for CI or operator artifacts:

```bash
python3 scripts/summarize_gui_diagnostics.py --report ./tmp/gui-diagnostics.json --output ./tmp/gui-diagnostics-summary.md
```

The production gate publishes that markdown summary directly into the GitHub Actions job summary and also uploads it as an artifact.

Headless dashboard smoke can be exercised with the same path used in CI:

```bash
QT_QPA_PLATFORM=offscreen xvfb-run -a ./.venv/bin/python third_party/mohawk_gui/test_dashboard.py
```

Render a compact markdown summary from that smoke output:
...
- `GHOSTLINK_PYTHON`: overrides the Python executable used by `ghost-link gui`. When unset, Ghostlink prefers the repository `.venv/bin/python` and only falls back to `python3` if no workspace virtualenv is present.
