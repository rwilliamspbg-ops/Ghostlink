...
> Archived: this was previously checked in as the repo root's `AGENTS.md`.
> It's a dated development-session log, not repo-wide agent instructions,
> so it's been moved here and `AGENTS.md` replaced with real contributor/
> agent guidance. Kept for historical context only — do not treat anything
> below as current.
...
2. Audit log is in-memory only — resets on restart. A persistent
   append-only trail is a bigger follow-up, not done here. (Note: this was
   since superseded — see the durable, exportable audit trail added in the
   v2.0.0 changelog.)
