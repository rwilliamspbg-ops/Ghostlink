...
- **Full GPU offload was ~2x-duplicating large models in system RAM** (`native_engine.rs`, `launch-native.ps1`, `main.rs`): the fix above was validated only against a small model. Live-testing it against the actual 30B-class daily-driver model (13.6GB) surfaced this — and a second bug masking it. On an integrated GPU, "VRAM" is the same physical RAM as everything else; llama.cpp's Vulkan backend offloading a layer doesn't move its weights out of system RAM the way it would on a discrete GPU, it *duplicates* them into a separate device-local allocation. Measured directly: 0.54GB / 7.35GB / 14.15GB committed memory at `ngl` 0 / 24 / -1 for the same model, for only 6.48 / 7.54 / ~18 tok/s — full offload left a 27.6GB host under 1GB free. `get_ngl()` now caps large models toward CPU-only automatically (still overridable via `GHOSTLINK_LLAMA_NGL`). This was masked in testing by two separate unconditional overrides — `launch-native.ps1` pinning `GHOSTLINK_LLAMA_NGL=-1` and `main.rs`'s own startup auto-config guessing an ngl from VRAM alone before any model was chosen — both removed, since either one alone made itself indistinguishable from a genuine user override and defeated the per-model capping. Also made `get_ctx_size()` cap context down for large models the same way (real headroom, though it turned out not to be the dominant cost here).
...
- All of the above verified live against a real running stack (Rust `ghost-link serve` release build + Go control-plane + Vite dev server), not just automated tests: real chat completions with real streamed tokens, real MCP server add/edit/delete/toggle round-trips, real model list deduplication against the actual `models.json` on disk, and — for the memory-duplication fix specifically — real `Get-Process`/`Win32_OperatingSystem` memory measurements before and after against the actual 30B model, not simulated.
