...
class ModelManager:
    ...
    def start_periodic_refresh(self):
        ...
        self.refresh_timer = threading.Timer(30, self.start_periodic_refresh)
    ...
    def load_ollama_models(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                current_models = set()
                for m in models:
                    current_models.add(m['name'].split(':')[0])
                self.loaded_models = current_models
        except Exception:
            # Silently fail if Ollama is not reachable
            pass
    ...
    def download_model(self, model_id: str):
        ...
        if model_id in self.downloading and self.downloading[model_id]['status'] == 'downloading':
...
class ModelManagementHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        if '/health' not in args[0]:
            print(f'[ModelManager] {format % args}')

if __name__ == '__main__':
    port = 8001
    server = HTTPServer(('127.0.0.1', port), ModelManagementHandler)
    print(f'[ModelManager] Running on http://127.0.0.1:{port}')
    try:
